angular.module('munimapAdmin')
    .controller('UserEditController', ['$scope', '$routeParams', 'ModelService', 'uibDateParser',
        function($scope, $routeParams, ModelService, uibDateParser) {
            ...
            $scope.submit = function() {
                ...
                if(angular.isDefined(_userConfig.mb_user_id)) {
                    ModelService.updateUser(_userConfig).then(function(data) {
                        if (!data.success) {
                            ...
                            $scope.userConfig.mb_user_password = undefined;
                        }
                    });
                } else {
                    ModelService.addUser(_userConfig, false).then(function(data) {
                        if (!data.success) {
                            angular.forEach(data.errors, function(idx, name) {
                                var errorElement = angular.element(document.querySelector( '#'+ name +'_error' ) );
                                errorElement.html(idx);
            ...
            if(angular.isDefined($routeParams.userId)) {
                ...
                ModelService.loadUser(_config, false).then(function(data) {
                    $scope.loading = false;
